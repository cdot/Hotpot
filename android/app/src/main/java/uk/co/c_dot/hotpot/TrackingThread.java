 ...
 */
class TrackingThread extends Thread implements SharedPreferences.OnSharedPreferenceChangeListener {

    /**
     * Loggin tag
     */
    private static final String TAG = "HOTPOT/TrackingThread";

    /**
     * Intent used internally for geofencing events
     */
    ...
    /**
     * Construct the service thread object
     *
     * @param context the Service
     */
    public TrackingThread(Context context) {
    ...
    /**
     * * A fatal error occurred; terminate the thread, telling the UI (and anyone else
    ...
    private void abort(String mess) {
        ...
        Intent i = new Intent(TrackingService.STOPPING);
    ...
    public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
        switch (key) {
            case TrackingService.PREF_URL:
                ...
                ed.remove(TrackingService.PREF_URL_WARNING);
                ed.remove(TrackingService.PREF_CERTS);
                if (sURL != null) {
                    try {
                        ...
                        if (certs != null && certs.size() > 0) {
                            ...
                            ed.putStringSet(TrackingService.PREF_CERTS, new HashSet<>(certs));
                        } else if (serverConnection.isSSL()) {
                            ed.putString(TrackingService.PREF_URL_WARNING,
                    ...
                    .setResultCallback(new ResultCallback<Status>() {
                        public void onResult(@NonNull Status r) {
                            if (!r.isSuccess())
                                ...
                                Intent i = new Intent(TrackingService.STARTED);
                                mServiceContext.sendBroadcast(i);
                            }
                        }
                    });
    ...
    public void run() {
        ...
        ifilt.addAction(TrackingService.STOP);
        ifilt.addAction(TrackingService.BOOST_HW);
        ifilt.addAction(TrackingService.BOOST_CH);
        mServiceContext.registerReceiver(new BroadcastListener(), ifilt);

        // PendingIntent for geofencing events.
    ...
    /**
     * Handler for a response from the /mobile/config call
     */
    private class ConfigResponseHandler implements ServerConnection.ResponseHandler {
        public void done(Object data) {
            ...
            try {
                ...
                Intent intent = new Intent(TrackingService.HOME_CHANGED);
                intent.putExtra("POS", pos);
                JSONObject fences = jd.getJSONObject("fences");
                intent.putExtra("FENCES", fences.toString());
                mServiceContext.sendBroadcast(intent);
                // We know where home is, so we can start fencing.
                startFencing(pos, fences);
            } catch (JSONException je) {
    ...
    private void establishConnection() throws SecurityException {
        ...
        String sURL = prefs.getString(TrackingService.PREF_URL, null);
        ...
        try {
            Set<String> certs = prefs.getStringSet(TrackingService.PREF_CERTS, null);
            ...
            mServerConnection.POST("/mobile/config", params, new ConfigResponseHandler());
    ...
    private void sendTransition(String fence, String transition, Location pos) {
        ...
        Intent intent = new Intent(TrackingService.FENCE_CROSSED);
        intent.putExtra("POS", pos);
    ...
    /**
     * Nested object used for handling connection callbacks
     */
    ...
    /**
     * Receiver for broadcasts coming from the UI
     */
    private class BroadcastListener extends BroadcastReceiver {
        @Override
        public void onReceive(Context cxt, Intent i) {
            switch (i.getAction()) {
                case TrackingService.BOOST_HW:
                    sendRequest("HW", 2);
                    break;
                case TrackingService.BOOST_CH:
                    sendRequest("CH", 2);
                    break;
                case TrackingService.STOP:

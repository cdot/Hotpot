 ...
 * class, as required by `import`. For example, given the model
 * ```
 * {
 *   thing: {
 *     $instantiable: true,
 ...
 * }
 * ```
 * when this data is read, it will use `import` to load the
  ...
  static remodel(args) {
    ...
    .then(rebuilt => {
      ...
      if (model.$instantiable) {
        ...
        // original data
        return import(`../../${rebuilt.$instance_of}.js`)
        .then(mod => {
          const cn = rebuilt.$instance_of.replace(/^.*\//, "");
          const module = mod[cn];
          let promise;
